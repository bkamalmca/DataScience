...
def get_model(learning_rate, maxlen):
    ...
    with mirrored_strategy.scope():
        embedding_layer = tf.keras.layers.Embedding(max_features,
                                                    embedding_dims,
                                                    input_length=maxlen)
        # embedding will take as input an integer matrix of size of max given and uses GPU
        ...
        x = tf.keras.layers.Dense(64, activation='relu')(embedded_sequences)
        #x = tf.keras.layers.Dropout(0.2)(x)
        preds = tf.keras.layers.Dense(1, activation='relu')(x)
